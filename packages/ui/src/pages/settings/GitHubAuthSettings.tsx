...
export function GitHubAuthSettings({ authClient, onSignedOut }: GitHubAuthSettingsProps) {
  ...
  const statusText = loading ? t("settings.auth.checkingAuth") : session?.state === "authenticated" ? t("settings.auth.authed") : t("settings.auth.needReauth");

  return (
    <Card>
      <CardHeader><CardTitle>{t("settings.auth.title")}</CardTitle><CardDescription>{t("settings.auth.description")}</CardDescription></CardHeader>
      <CardContent className="space-y-4">
        <div className="flex items-center gap-3 rounded-lg border border-[var(--border-default)] bg-[var(--bg-muted)] p-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="truncate text-sm font-medium text-[var(--text-primary)]">{loading ? t("settings.auth.reading") : displayName}</div>
            <div className="truncate text-xs text-[var(--text-secondary)]">{statusText}</div>
          </div>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row">
          <Button variant="outline" onClick={handleReauthorize} disabled={pendingAction !== null}><RefreshCw size={16} />{pendingAction === "reauthorize" ? t("settings.auth.authorizing") : t("settings.auth.reauthorize")}</Button>
          <Button variant="secondary" onClick={handleSignOut} disabled={pendingAction !== null}><LogOut size={16} />{pendingAction === "signOut" ? t("settings.auth.signingOut") : t("settings.auth.signOut")}</Button>
        </div>
        {deviceFlow && (
          <div className="rounded-lg border border-[var(--border-default)] bg-[var(--bg-muted)] p-3 text-center">
            ...
            <a href={deviceFlow.verificationUri} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center justify-center gap-2 text-xs font-medium text-[var(--brand-primary)] hover:underline">{t("settings.auth.openGitHub")}<ArrowRight size={12} /></a>
          </div>
        )}
      </CardContent>
    </Card>
