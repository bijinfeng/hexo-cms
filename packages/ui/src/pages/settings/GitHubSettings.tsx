...
export function GitHubSettings({ authClient, onSignedOut }: GitHubSettingsProps) {
    ...
    async function loadConfig() {
      try {
        ...
        if (configData) {
          ...
          setWorkflowFile(
            configData.workflow_file || configData.workflowFile || ".github/workflows/deploy.yml",
          );
          setAutoDeploy(configData.auto_deploy === 1 || configData.autoDeploy === true);
          setDeployNotifications(
            configData.deploy_notifications === 1 || configData.deployNotifications === true,
          );
        }
      } catch (error) {
  ...
  if (loading)
    return <div className="text-sm text-[var(--text-secondary)]">{t("common.loading")}</div>;
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <FormField
            label={t("settings.github.repoAddress")}
            description={t("settings.github.repoFormat")}
          >
          ...
          <FormField label={t("settings.github.branch")}>
            <Input type="text" value={branch} onChange={(e) => setBranch(e.target.value)} />
          </FormField>
          <FormField
            label={t("settings.github.postsDir")}
            description={t("settings.github.postsDirHint")}
          >
            <Input
              type="text"
              value={postsDir}
          ...
          <FormField
            label={t("settings.github.mediaDir")}
            description={t("settings.github.mediaDirHint")}
          >
            <Input
              type="text"
              value={mediaDir}
          ...
          <Button onClick={handleSave} disabled={saving || !owner || !repo}>
            {saving ? t("common.saving") : t("settings.github.saveConfig")}
          </Button>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle>{t("settings.github.githubActions")}</CardTitle>
          <CardDescription>{t("settings.github.actionsDesc")}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <FormField
            label={t("settings.github.workflowFile")}
            description={t("settings.github.workflowHint")}
          >
            <Input
              type="text"
              value={workflowFile}
          ...
          <ToggleField
            label={t("settings.github.autoDeploy")}
            description={t("settings.github.autoDeployHint")}
            checked={autoDeploy}
            onChange={setAutoDeploy}
          />
          <ToggleField
            label={t("settings.github.deployNotify")}
            description={t("settings.github.deployNotifyHint")}
