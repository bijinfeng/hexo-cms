...
export function WindowControls() {
  ...
  function minimize() {
    getElectronAPI()?.invoke("window:minimize");
  }
  function toggleMaximize() {
    getElectronAPI()?.invoke(maximized ? "window:unmaximize" : "window:maximize");
  }
  function close() {
    getElectronAPI()?.invoke("window:close");
  }

  const btnClass =
    "w-8 h-8 flex items-center justify-center rounded-md text-[var(--text-tertiary)] hover:bg-[var(--bg-muted)] hover:text-[var(--text-primary)] transition-colors cursor-pointer";
