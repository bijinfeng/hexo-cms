...
function mapRepository(repo: GitHubRepo): RepositoryOption {
  const fallbackFullName = repo.full_name ?? "";
  const [fallbackOwner = "", fallbackName = ""] = fallbackFullName.split("/");
...
export async function validateHexoRepository(
  ...
  try {
    ...
    try {
      await octokit.rest.repos.getBranch({ owner: selection.owner, repo: selection.repo, branch });
    } catch (error) {
      if (!isGitHubNotFound(error)) throw error;
      return {
        ok: false, repository,
