...
import type { GitHubConfig, PluginConfigStoreValue, PluginLogStoreValue, PluginSecretStoreValue, PluginStateStoreValue, PluginStorageStoreValue, HexoPost } from "@hexo-cms/core";
...
export function registerIpcHandlers(deps: Dependencies) {
  ...
  const logErr = (channel: string, ctx?: Record<string, unknown>) => (error: unknown) => {
    console.error(JSON.stringify({ level: "error", message: `IPC: ${channel} failed`, ...ctx, error: String(error) }));
  };
  ...
  ipcMain.handle("auth:signOut", async () => { await desktopAuth.signOut(); });
  ...
  ipcMain.handle("config:save", (_event, config: GitHubConfig) => {
    try {
      ...
      return true;
    } catch (error) { logErr("config:save")(error); throw error; }
  });
  ...
  ipcMain.handle("github:read-config-file", async (_event, configPath: string) => {
    ...
    } catch (error) { logErr("github:read-config-file", { path: configPath })(error); throw error; }
  });
  ...
  ipcMain.handle("github:write-config-file", async (_event, configPath: string, content: string) => {
    ...
    } catch (error) { logErr("github:write-config-file", { path: configPath })(error); throw error; }
  });

  // Plugin stores
  ipcMain.handle("plugin-storage:load", () => desktopPersistence.loadPluginStorage());
  ipcMain.handle("plugin-storage:save", (_event, value: PluginStorageStoreValue) => desktopPersistence.savePluginStorage(value));
  ipcMain.handle("plugin-secret:load", async () => ({}));
  ipcMain.handle("plugin-secret:save", async (_event, value: PluginSecretStoreValue) => { await desktopPersistence.savePluginSecrets(value); });
  ipcMain.handle("plugin-secret:has", async (_event, { pluginId, key }: { pluginId: string; key: string }) => desktopPersistence.hasPluginSecret(pluginId, key));
  ipcMain.handle("plugin-secret:mutate", async (_event, mutation: PluginSecretMutation) => { await desktopPersistence.mutatePluginSecret(mutation); });
  ipcMain.handle("plugin-http:fetch", async (_event, req: PluginFetchRequest) => pluginHttpProxy.fetch(req));
  ipcMain.handle("plugin-network-audit:list", async (_event, limit?: number) => desktopPersistence.listPluginNetworkAudit(limit));
  ipcMain.handle("plugin-state:load", () => desktopPersistence.loadPluginState());
  ipcMain.handle("plugin-state:save", (_event, value: PluginStateStoreValue) => desktopPersistence.savePluginState(value));
  ipcMain.handle("plugin-config:load", () => desktopPersistence.loadPluginConfig());
  ipcMain.handle("plugin-config:save", (_event, value: PluginConfigStoreValue) => desktopPersistence.savePluginConfig(value));
  ipcMain.handle("plugin-logs:load", () => desktopPersistence.loadPluginLogs());
  ipcMain.handle("plugin-logs:save", (_event, value: PluginLogStoreValue) => desktopPersistence.savePluginLogs(value));
  ...
  ipcMain.handle("onboarding:listRepositories", async (_event, input: { query?: string } = {}) => {
    ...
    return listWritableRepositories(new Octokit({ auth: token, headers: { "X-GitHub-Api-Version": GITHUB_API_VERSION } }) as OctokitLike, input);
  });

  ipcMain.handle("onboarding:validateRepository", async (_event, input: { owner: string; repo: string; branch?: string }) => {
    const token = await desktopAuth.getAccessToken();
    if (!token) return { ok: false, checks: [{ id: "access", status: "error", message: "当前授权缺少仓库读写权限，请重新授权" }], error: "REAUTH_REQUIRED" };
    const { Octokit } = await import("octokit");
    return validateHexoRepository(new Octokit({ auth: token, headers: { "X-GitHub-Api-Version": GITHUB_API_VERSION } }) as OctokitLike, input);
  });
  ...
  ipcMain.handle("github:get-posts", async () => {
    ...
    } catch (error) { logErr("github:get-posts")(error); return []; }
  });

  ipcMain.handle("github:get-post", async (_event, path: string) => {
    try { return await (await gh()).getPost(path); }
    catch (error) { logErr("github:get-post", { path })(error); throw error; }
  });

  ipcMain.handle("github:save-post", async (_event, post: HexoPost) => {
    try { await (await gh()).savePost(post); }
    catch (error) { logErr("github:save-post", { path: post.path })(error); throw error; }
  });

  ipcMain.handle("github:delete-post", async (_event, path: string) => {
    try { await (await gh()).deletePost(path); }
    catch (error) { logErr("github:delete-post", { path })(error); throw error; }
  });
  ...
  ipcMain.handle("github:get-pages", async () => {
    ...
    } catch (error) { logErr("github:get-pages")(error); return []; }
  });

  ipcMain.handle("github:get-page", async (_event, path: string) => {
    try { return await (await gh()).getPost(path); }
    catch (error) { logErr("github:get-page", { path })(error); throw error; }
  });

  ipcMain.handle("github:save-page", async (_event, post: HexoPost) => {
    try { await (await gh()).savePost(post); }
    catch (error) { logErr("github:save-page", { path: post.path })(error); throw error; }
  });

  ipcMain.handle("github:delete-page", async (_event, path: string) => {
    try { await (await gh()).deletePost(path); }
    catch (error) { logErr("github:delete-page", { path })(error); throw error; }
  });
  ...
  ipcMain.handle("github:get-media", async () => {
    ...
    try {
      ...
      return files.map((f) => ({ name: f.name, path: f.path, size: 0, url: `https://github.com/${config?.owner}/${config?.repo}/blob/${config?.branch || "main"}/${f.path}`, sha: "" }));
    } catch (error) { logErr("github:get-media")(error); return []; }
  });
  ...
  ipcMain.handle("github:upload-media", async (_event, { buffer, path, name }: { buffer: ArrayBuffer; path: string; name: string; type: string }) => {
    ...
    } catch (error) { logErr("github:upload-media", { path, name })(error); throw new Error("Failed to upload media", { cause: error }); }
  });

  ipcMain.handle("github:delete-media", async (_event, path: string) => {
    try { await (await gh()).deleteMedia(path); }
    catch (error) { logErr("github:delete-media", { path })(error); throw new Error("Failed to delete media", { cause: error }); }
  });
  ...
  ipcMain.handle("github:get-stats", async () => {
    ...
    return { totalPosts: posts.length, publishedPosts: posts.filter((p) => !p.frontmatter.draft).length, draftPosts: posts.filter((p) => p.frontmatter.draft).length, totalViews: 0 };
  });
  ...
  ipcMain.handle("github:get-themes", async () => {
    ...
    try {
      ...
      const installedThemes = themeEntries.filter((e) => e.type === "dir").map((e) => ({ name: e.name, path: e.path }));
      return { currentTheme: currentTheme || "", installedThemes };
    } catch { return { currentTheme: "", installedThemes: [] }; }
  });
  ...
  ipcMain.handle("github:switch-theme", async (_event, themeName: string) => {
    try {
      ...
      await github.writeRawFile("_config.yml", setYamlScalar(configFile.content, "theme", themeName), `切换主题为: ${themeName}`);
    } catch (error) { logErr("github:switch-theme", { themeName })(error); throw error; }
  });
  ...
  ipcMain.handle("github:get-deployments", async () => {
    ...
    try {
      const { Octokit } = await import("octokit");
      const octokit = new Octokit({ auth: token, headers: { "X-GitHub-Api-Version": GITHUB_API_VERSION } });
      const { data } = await octokit.rest.actions.listWorkflowRunsForRepo({ owner: config.owner, repo: config.repo, per_page: 20 });
  ...
  ipcMain.handle("github:trigger-deploy", async (_event, workflowFile: string) => {
    ...
    try {
      ...
      const { Octokit } = await import("octokit");
      const octokit = new Octokit({ auth: token, headers: { "X-GitHub-Api-Version": GITHUB_API_VERSION } });
      await octokit.rest.actions.createWorkflowDispatch({ owner: config.owner, repo: config.repo, workflow_id: workflowFile, ref: config.branch || "main" });
    } catch (error) { logErr("github:trigger-deploy", { workflowFile })(error); throw error; }
  });
}
