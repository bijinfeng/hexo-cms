...
describe("desktop taxonomy operations", () => {
  ...
  it("renames tag and category values without mutating unchanged posts", () => {
    ...
    expect(
      renamePostTaxonomy(source, { type: "tag", oldName: "CMS", newName: "Content" })?.frontmatter,
    ).toEqual({ tags: ["Hexo", "Content"], category: "Dev", categories: "Old" });
    expect(
      renamePostTaxonomy(source, { type: "category", oldName: "Old", newName: "Archive" })
        ?.frontmatter,
    ).toEqual({ tags: ["Hexo", "CMS"], category: "Dev", categories: "Archive" });
    expect(
      renamePostTaxonomy(source, { type: "tag", oldName: "Missing", newName: "Next" }),
    ).toBeNull();
  ...
  it("deletes tag and category values", () => {
    expect(
      deletePostTaxonomy(post("a.md", { tags: ["Hexo", "CMS"] }), { type: "tag", name: "Hexo" })
        ?.frontmatter,
    ).toEqual({ tags: ["CMS"] });
    expect(
      deletePostTaxonomy(post("b.md", { tags: "Hexo" }), { type: "tag", name: "Hexo" })
        ?.frontmatter,
    ).toEqual({ tags: [] });
    expect(
      deletePostTaxonomy(post("c.md", { category: "Dev", categories: "Notes" }), {
        type: "category",
        name: "Dev",
      })?.frontmatter,
    ).toEqual({ categories: "Notes" });
    expect(
      deletePostTaxonomy(post("d.md", { category: "Dev" }), { type: "category", name: "Missing" }),
    ).toBeNull();
  });
  ...
  it("persists only changed posts for rename and delete operations", async () => {
    ...
    await expect(
      renameTaxonomy(repository, { type: "tag", oldName: "Hexo", newName: "Blog" }),
    ...
    savePost.mockClear();
    await expect(deleteTaxonomy(repository, { type: "tag", name: "CMS" })).resolves.toEqual({
      updatedCount: 1,
    });
