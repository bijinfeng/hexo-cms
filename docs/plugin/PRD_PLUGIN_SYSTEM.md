...
19. 已覆盖 core 插件单元测试、UI 插件管理测试、配置持久化测试、媒体边界测试、布局边界测试、ErrorBoundary 测试、Sidebar/Command/Storage/Event/Log 测试和 switch 组件回归测试。

v0.2 仍需继续补齐:

1. Secret Store、受控网络代理和第三方插件沙箱。
...
状态: 部分落地，继续迭代；日志面板、页面事件派发和 Storage 平台持久化已完成，下一步推进 Secret Store 与受控 network.fetch。
...
1. Secret Store、受控 network.fetch 和沙箱验证。
2. Web SQLite / Desktop userData 的平台级状态、配置和日志存储。

下一轮迭代建议:

1. 增加 Secret Store 基础 API，只暴露 has/set/delete，不向插件返回明文。
2. 增加受控 network.fetch 最小实现，先限制 HTTPS、allowedHosts、超时和 cookie。
