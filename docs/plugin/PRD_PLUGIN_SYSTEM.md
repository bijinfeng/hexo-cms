...
14. 已覆盖 core 插件单元测试、UI 插件管理测试、配置持久化测试、媒体边界测试、布局边界测试、ErrorBoundary 测试、Sidebar/Command 测试和 switch 组件回归测试。

v0.2 仍需继续补齐:

1. Event API 的只读通知通道。
2. 插件日志面板。
3. 插件 Storage API 与跨平台持久化适配。当前配置持久化仍是 renderer 侧 localStorage，后续 Web 应落到服务端/SQLite，Desktop 应落到 userData。
...
1. 先实现插件 Storage API 的 Memory/Browser store 与权限隔离，验证插件级 namespace。
2. 实现 Event API 的只读发布/订阅通道，先覆盖文章保存、媒体上传、部署状态变化等宿主事件。
3. 增加插件日志面板，按 `pluginId` 展示命令执行、运行时错误和后续事件处理日志。
