...
截至 2026-05-10，插件系统 v0.1 MVP 与 v0.2 首批能力已完成:
...
9. 已覆盖 core 插件单元测试、UI 插件管理测试、配置持久化测试、媒体边界测试、布局边界测试和 switch 组件回归测试。

v0.2 仍需继续补齐:

1. 插件级 ErrorBoundary 和最近错误展示。这是下一轮 P0，用来保证插件渲染失败不导致核心页面白屏。
2. Sidebar item 实际挂载。
3. Command/Event API 的执行通道。
4. 插件日志面板和错误阈值熔断。
5. 插件 Storage API 与跨平台持久化适配。当前配置持久化仍是 renderer 侧 localStorage，后续 Web 应落到服务端/SQLite，Desktop 应落到 userData。
...
1. 插件级 ErrorBoundary 与最近错误展示。
2. Sidebar entry 扩展点实际挂载。
3. Command API。
4. Event API。
5. 插件 Storage API。
6. 插件日志面板。
7. 错误阈值熔断。
8. Web SQLite / Desktop userData 的平台级配置存储。

下一轮迭代建议:

1. 先实现插件级 ErrorBoundary，覆盖 Dashboard widget 和 Settings schema renderer。
2. 将 renderer 错误写入 `record.lastError`，Settings 中展示“最近错误”和停用入口。
3. 添加回归测试，证明插件 renderer 抛错时 Dashboard、Settings 和核心页面仍可用。
...
7. 当前配置持久化先使用 renderer localStorage 验证产品体验，平台级持久化再分别落到 Web SQLite 与 Desktop userData。
