...
16. 已覆盖 core 插件单元测试、UI 插件管理测试、配置持久化测试、媒体边界测试、布局边界测试、ErrorBoundary 测试、Sidebar/Command/Storage/Event 测试和 switch 组件回归测试。

v0.2 仍需继续补齐:

1. 插件日志面板。
2. 插件 Storage API 的平台持久化适配。当前配置和 storage core 仍主要使用 renderer 侧 localStorage，后续 Web 应落到服务端/SQLite，Desktop 应落到 userData。
...
1. 增加插件日志面板，按 `pluginId` 展示命令执行、运行时错误和后续事件处理日志。
2. 在核心页面中接入宿主事件派发，先覆盖文章保存、媒体上传、部署触发/状态变化。
