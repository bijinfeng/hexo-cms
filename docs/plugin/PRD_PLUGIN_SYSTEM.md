...
18. 已覆盖 core 插件单元测试、UI 插件管理测试、配置持久化测试、媒体边界测试、布局边界测试、ErrorBoundary 测试、Sidebar/Command/Storage/Event/Log 测试和 switch 组件回归测试。

v0.2 仍需继续补齐:

1. 插件 Storage API 的平台持久化适配。当前配置、storage 和 log store 仍主要使用 renderer 侧 localStorage，后续 Web 应落到服务端/SQLite，Desktop 应落到 userData。
2. Secret Store、受控网络代理和第三方插件沙箱。
...
状态: 部分落地，继续迭代；日志面板和页面事件派发已完成，下一步推进平台持久化。
...
1. 插件 Storage API 的平台持久化适配。
2. Web SQLite / Desktop userData 的平台级配置存储。
3. Secret Store、受控 network.fetch 和沙箱验证。

下一轮迭代建议:

1. 继续将插件 Storage API 从 renderer localStorage 迁移到 Web SQLite 与 Desktop userData。
